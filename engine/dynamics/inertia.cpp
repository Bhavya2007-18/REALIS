...
Mat3 Inertia::cylinder(float mass, float radius, float height) {
    ...
    float i_trans = (1.0f / 12.0f) * mass * (3 * radius * radius + height * height);
    
    Mat3 m;
    // Assuming Y is up/axis
    m.data[0] = i_trans; m.data[1] = 0;       m.data[2] = 0;
    m.data[3] = 0;       m.data[4] = i_axial; m.data[5] = 0;
    m.data[6] = 0;       m.data[7] = 0;       m.data[8] = i_trans;
    return m;
}
