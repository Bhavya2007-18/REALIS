...
void run_collision_test() {
    std::cout << "=== Phase 4: Collision & Impulse Resolution Test ===" << std::endl;
    ...
    std::cout << "Initial Velocities: A=" << body_a.velocity.x << ", B=" << body_b.velocity.x << std::endl;
    std::cout << "Initial Momentum: " << (body_a.mass * body_a.velocity.x + body_b.mass * body_b.velocity.x) << std::endl;

    bool collision_occurred = false;

    for (int i = 0; i < steps; ++i) {
        // 1. Detection
        Contact contact = CollisionDetector::get_contact(body_a, body_b);
        
        if (contact.colliding) {
            if (!collision_occurred) {
                std::cout << "[Step " << i << "] COLLISION detected! t=" << i*dt << "s" << std::endl;
    ...
    std::cout << "Final Velocities: A=" << body_a.velocity.x << ", B=" << body_b.velocity.x << std::endl;
    std::cout << "Final Momentum: " << (body_a.mass * body_a.velocity.x + body_b.mass * body_b.velocity.x) << std::endl;
    ...
    bool momentum_ok = std::abs((body_a.mass * body_a.velocity.x + body_b.mass * body_b.velocity.x) - 2.0f) < 0.01f;

    if (v1_ok && v2_ok && momentum_ok) {
        std::cout << "✓ PASS: Collision response matches Python/analytical laws of momentum & restitution!" << std::endl;
    } else {
        std::cout << "✗ FAIL: Physical laws violated or deviation too high" << std::endl;
    }
}
