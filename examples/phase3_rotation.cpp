...
void run_angular_stability_test() {
    std::cout << "=== Phase 3: Angular Stability (Tennis Racket Effect) ===" << std::endl;
    ...
    std::cout << "Initial Angular Vel: (" << book.angular_velocity.x << ", " 
              << book.angular_velocity.y << ", " << book.angular_velocity.z << ")" << std::endl;
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        if (book.angular_velocity.y * last_wy < 0) {
            flips++;
            std::cout << "[Step " << i << "] FLIP detected! t=" << i*dt << "s, wy=" << book.angular_velocity.y << std::endl;
        }
        last_wy = book.angular_velocity.y;

        // Still call integrate to update orientation (though not strictly needed for this specific test)
        book.integrate(dt);
    }
    ...
    if (flips >= 2) {
        std::cout << "✓ PASS: Dzhanibekov effect reproduced (Rotation is unstable about intermediate axis)" << std::endl;
    } else {
