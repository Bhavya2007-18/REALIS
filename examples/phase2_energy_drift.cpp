...
void run_drift_analysis() {
    std::cout << "=== Phase 2: Integrator Energy Drift Comparison ===" << std::endl;
    ...
    auto calc_osc_energy = [](const RigidBody& b) {
        return 0.5f * b.velocity.dot(b.velocity) + 0.5f * b.position.dot(b.position);
    };
    ...
    if (std::abs(drift) < 0.1f) {
        std::cout << "✓ PASS: Energy drift within tolerance for Symplectic Integrator" << std::endl;
    } else {
        std::cout << "✗ FAIL: Energy drift too high" << std::endl;
    }

    // Comparison with Forward Euler logic (implemented manually here to avoid changing core for now)
    std::cout << std::endl << "--- Comparison: Forward Euler (Explicit) ---" << std::endl;
    ...
    std::cout << "(Note: Forward Euler energy grows significantly, as expected from Python validation)" << std::endl;
}
